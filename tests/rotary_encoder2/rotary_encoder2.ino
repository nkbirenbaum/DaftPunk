...
// This sketch implements software debounce, but you can further improve performance by placing 0.1uF capacitors between A and ground, and B and ground.
// Note that because this is a common anode device, the pushbutton requires an external 1K-10K pullDOWN resistor to operate.

// Rotary encoder pin A to digital pin 3* (do not change this pin because of interrupts, others may change)
// Rotary encoder pin B to analog pin 3
// Rotary encoder pin C to ground
// Rotary encoder pin 1 (red cathode) to D9 (old: digital pin 10)
// Rotary encoder pin 2 (green cathode) to D8 (old: analog pin 1)
// Rotary encoder pin 3 (button) to digital pin 4
// Rotary encoder pin 4 (blue cathode) to D7 (old: digital pin 5)
// Rotary encoder pin 5 (common anode) to VCC (3.3V or 5V)

// HOW IT WORKS
// The I/O pins used by the rotary encoder hardware are set up to automatically call interrupt functions (rotaryIRQ and buttonIRQ) each time the rotary encoder changes states.
// There is also code in the main loop() that keeps track of whether the button is currently being held down and for how long. This is useful for "hold button down for five seconds
// to power off"-type situations, which cannot be handled by interrupts alone because no interrupts will be called until the button is actually released.

#include <PinChangeInt.h>

// LilyPad MP3 pin defines. Not all of these are used in this sketch; the unused pins are commented out:
...
void setup()
{
  ...
  // We use the standard external interrupt pin for the rotary, but we'll use the pin change interrupt library for the button.
...
void loop()
{
  // The rotary IRQ sets the flag rotary_counter to true if the knob position has changed. We can use this flag to do something in the main loop() each time there's
  // a change. We'll clear this flag when we're done, so that we'll only do this if() once for each change.
  if (rotary_change)
  {
    Serial.print("rotary: ");
  ...
  // The button IRQ also sets flags to true, one for button_pressed, one for button_released. Like the rotary flag, we'll clear these when we're done handling them.
  if (button_pressed)
  {
    ...
    // We'll set another flag saying the button is now down this is so we can keep track of how long the button is being held down. (We can't do this in interrupts,
...
// The rotaryIRQ function transparently maintains a counter that increments or decrements by one for each detent ("click") of the rotary encoder knob. This function also sets a flag
// (rotary_change) to true whenever the counter changes. You can check this flag in your main loop() code and perform an action when the knob is turned.
void rotaryIRQ()
{
...
// The buttonIRQ function does the same thing for the pushbutton built into the rotary encoder knob. It will set flags for button_pressed and button_released that you can monitor in your
// main loop() code. There is also a variable for button_downtime which records how long the button was held down.
void buttonIRQ()
{
